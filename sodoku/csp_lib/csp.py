...
def backtracking_search(csp,
    ...
    def backtrack(assignment):
        "backtrack - Helper function with current set of assignments"
        
        if len(assignment) == len(csp.vars):
            return assignment   # Everything has been assigned
        
        # Pick variable that we will work on
        var = select_unassigned_variable(assignment, csp)
        # Pick value within variable
        for value in order_domain_values(var, assignment, csp):
            # Does this value produce a conflict?
            if 0 == csp.nconflicts(var, value, assignment):
                # No, go ahead and assign it
                csp.assign(var, value, assignment)
                # Add the assignment to the hypothesis dictionary
                # (curr_domains) and note the values that we are excluding
                removals = csp.suppose(var, value)
                # Run any forward inference algorithm to check
                # whether or not constraints hold.  If they don't
                # restore the CSP to the previous state.  Otherwise, 
                # run the backtracker with the current assignment.
